...
-- | display the config
instance P.Show ArgoConfig where
  show c =
    T.printf
...
-- | constructs a default ArgoConfig
--
-- configHost:
--
-- @https://localhost@
...
-- @"argo/0.1.0.0"@
--
newConfig :: IO ArgoConfig
newConfig = do
    logCxt <- initLogContext
    return $ ArgoConfig
        { configHost = "https://localhost"
        , configUserAgent = "argo/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => ArgoConfig -> auth -> ArgoConfig
addAuthMethod config@ArgoConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: ArgoConfig -> IO ArgoConfig
...
withStderrLogging :: ArgoConfig -> IO ArgoConfig
...
withNoLogging :: ArgoConfig -> ArgoConfig
...
data ArgoRequest req contentType res accept = ArgoRequest
  { rMethod  :: NH.Method   -- ^ Method of ArgoRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of ArgoRequest
  , rParams   :: Params -- ^ params of ArgoRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (ArgoRequest req contentType res accept) NH.Method
rMethodL f ArgoRequest{..} = (\rMethod -> ArgoRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (ArgoRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f ArgoRequest{..} = (\rUrlPath -> ArgoRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (ArgoRequest req contentType res accept) Params
rParamsL f ArgoRequest{..} = (\rParams -> ArgoRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (ArgoRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f ArgoRequest{..} = (\rAuthTypes -> ArgoRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => ArgoRequest req contentType res accept -> param -> ArgoRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: ArgoRequest req contentType res accept -> param -> ArgoRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: ArgoRequest req contentType res accept -> param -> ArgoRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method 
          ...
          -> ArgoRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = ArgoRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: ArgoRequest req contentType res accept -> [NH.Header] -> ArgoRequest req contentType res accept
setHeader req header =
  req `removeHeader` P.fmap P.fst header &
  L.over (rParamsL . paramsHeadersL) (header P.++)

removeHeader :: ArgoRequest req contentType res accept -> [NH.HeaderName] -> ArgoRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => ArgoRequest req contentType res accept -> ArgoRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => ArgoRequest req contentType res accept -> ArgoRequest req contentType res accept
_setAcceptHeader req =
    case mimeType (P.Proxy :: P.Proxy accept) of 
        Just m -> req `setHeader` [("accept", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["accept"]

setQuery :: ArgoRequest req contentType res accept -> [NH.QueryItem] -> ArgoRequest req contentType res accept
setQuery req query = 
  req &
  L.over
...
addForm :: ArgoRequest req contentType res accept -> WH.Form -> ArgoRequest req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: ArgoRequest req contentType res accept -> NH.Part -> ArgoRequest req contentType res accept
...
_setBodyBS :: ArgoRequest req contentType res accept -> B.ByteString -> ArgoRequest req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: ArgoRequest req contentType res accept -> BL.ByteString -> ArgoRequest req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => ArgoRequest req contentType res accept -> P.Proxy authMethod -> ArgoRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    :: ArgoConfig
    -> a
    -> ArgoRequest req contentType res accept
    -> IO (ArgoRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> ArgoConfig
  -> IO (ArgoRequest req contentType res accept)
_applyAuthMethods req config@(ArgoConfig {configAuthMethods = as}) =
  foldlM go req as
  where
